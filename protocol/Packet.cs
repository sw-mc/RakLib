...
public abstract class Packet {
	...
	protected void EncodeHeader(PacketSerializer buffer) {
		buffer.WriteByte((byte) GetPid());
	}
	
	protected abstract void EncodePayload(PacketSerializer buffer);
	
	public void Encode(PacketSerializer buffer) {
		EncodeHeader(buffer);
		EncodePayload(buffer);
	}
	
	protected void DecodeHeader(PacketSerializer buffer) {
		buffer.ReadByte(); //PID
	}
	
	protected abstract void DecodePayload(PacketSerializer buffer);
	
	public void Decode(PacketSerializer buffer) {
		DecodeHeader(buffer);
...
public class PacketSerializer : BinaryStream {
	...
	public void WriteAddress(InternetAddress address) {
		var version = address.Version;
		Writer.Write((byte) version);
		if (version != InternetVersion.Ipv4)
			throw new BinaryDataException("Unknown address version " + version);
		foreach (var part in address.Ip.Split('.')) {
			Writer.Write(~int.Parse(part) & 0xff);
		}
...
public abstract class PacketReliability {
	...
	public const int MAX_ORDER_CHANNELS = 32;
	...
	public static bool IsOrdered(int reliability) {
		return reliability is RELIABLE_ORDERED or RELIABLE_ORDERED_WITH_ACK_RECEIPT;
	}
	
	public static bool IsSequencedOrOrdered(int reliability) {
		return reliability is UNRELIABLE_SEQUENCED or RELIABLE_SEQUENCED or RELIABLE_ORDERED or RELIABLE_ORDERED_WITH_ACK_RECEIPT;
	}
	
}

sealed class MessageIdentifiers{
	...
	public const int ID_OPEN_CONNECTION_REQUEST_1 = 0x05;
	...
	public const int ID_OPEN_CONNECTION_REPLY_1 = 0x06;
	...
	public const int ID_OUT_OF_BAND_INTERNAL = 0x0d;
	...
	/// the message arrived
	public const int ID_SND_RECEIPT_ACKED = 0x0e;
	...
	public const int ID_SND_RECEIPT_LOSS = 0x0f;
	...
	public const int ID_INCOMPATIBLE_PROTOCOL_VERSION = 0x19;
	...
	public const int ID_IP_RECENTLY_CONNECTED = 0x1a;
	...
	public const int ID_TIMESTAMP = 0x1b;
	...
	public const int ID_UNCONNECTED_PONG = 0x1c;
	...
	/// the data parameter
	public const int ID_ADVERTISE_SYSTEM = 0x1d;
	...
	public const int ID_DOWNLOAD_PROGRESS = 0x1e;
	...
	public const int ID_REMOTE_DISCONNECTION_NOTIFICATION = 0x1f;
	/// ConnectionGraph2 plugin - In a client/server environment, a client other than ourselves has been forcefully dropped.
	///  Packet::systemAddress is modified to reflect the systemAddress of this client.
	public const int ID_REMOTE_CONNECTION_LOST = 0x20;
	/// ConnectionGraph2 plugin: Bytes 1-4 = count. for (count items) contains {SystemAddress, RakNetGUID, 2 byte ping}
	public const int ID_REMOTE_NEW_INCOMING_CONNECTION = 0x21;

	/// FileListTransfer plugin - Setup data
	public const int ID_FILE_LIST_TRANSFER_HEADER = 0x22;
	/// FileListTransfer plugin - A file
	public const int ID_FILE_LIST_TRANSFER_FILE = 0x23;
	// Ack for reference push, to send more of the file
	public const int ID_FILE_LIST_REFERENCE_PUSH_ACK = 0x24;

	/// DirectoryDeltaTransfer plugin - Request from a remote system for a download of a directory
	public const int ID_DDT_DOWNLOAD_REQUEST = 0x25;

	/// RakNetTransport plugin - Transport provider message, used for remote console
	public const int ID_TRANSPORT_STRING = 0x26;

	/// ReplicaManager plugin - Create an object
	public const int ID_REPLICA_MANAGER_CONSTRUCTION = 0x27;
	/// ReplicaManager plugin - Changed scope of an object
	public const int ID_REPLICA_MANAGER_SCOPE_CHANGE = 0x28;
	/// ReplicaManager plugin - Serialized data of an object
	public const int ID_REPLICA_MANAGER_SERIALIZE = 0x29;
	/// ReplicaManager plugin - New connection, about to send all world objects
	public const int ID_REPLICA_MANAGER_DOWNLOAD_STARTED = 0x2a;
	/// ReplicaManager plugin - Finished downloading all serialized objects
	public const int ID_REPLICA_MANAGER_DOWNLOAD_COMPLETE = 0x2b;

	/// RakVoice plugin - Open a communication channel
	public const int ID_RAKVOICE_OPEN_CHANNEL_REQUEST = 0x2c;
	/// RakVoice plugin - Communication channel accepted
	public const int ID_RAKVOICE_OPEN_CHANNEL_REPLY = 0x2d;
	/// RakVoice plugin - Close a communication channel
	public const int ID_RAKVOICE_CLOSE_CHANNEL = 0x2e;
	/// RakVoice plugin - Voice data
	public const int ID_RAKVOICE_DATA = 0x2f;

	/// Autopatcher plugin - Get a list of files that have changed since a certain date
	public const int ID_AUTOPATCHER_GET_CHANGELIST_SINCE_DATE = 0x30;
	/// Autopatcher plugin - A list of files to create
	public const int ID_AUTOPATCHER_CREATION_LIST = 0x31;
	/// Autopatcher plugin - A list of files to delete
	public const int ID_AUTOPATCHER_DELETION_LIST = 0x32;
	/// Autopatcher plugin - A list of files to get patches for
	public const int ID_AUTOPATCHER_GET_PATCH = 0x33;
	/// Autopatcher plugin - A list of patches for a list of files
	public const int ID_AUTOPATCHER_PATCH_LIST = 0x34;
	/// Autopatcher plugin - Returned to the user: An error from the database repository for the autopatcher.
	public const int ID_AUTOPATCHER_REPOSITORY_FATAL_ERROR = 0x35;
	/// Autopatcher plugin - Returned to the user: The server does not allow downloading unmodified game files.
	public const int ID_AUTOPATCHER_CANNOT_DOWNLOAD_ORIGINAL_UNMODIFIED_FILES = 0x36;
	/// Autopatcher plugin - Finished getting all files from the autopatcher
	public const int ID_AUTOPATCHER_FINISHED_INTERNAL = 0x37;
	public const int ID_AUTOPATCHER_FINISHED = 0x38;
	/// Autopatcher plugin - Returned to the user: You must restart the application to finish patching.
	public const int ID_AUTOPATCHER_RESTART_APPLICATION = 0x39;

	/// NATPunchthrough plugin: internal
	public const int ID_NAT_PUNCHTHROUGH_REQUEST = 0x3a;
	/// NATPunchthrough plugin: internal
	//ID_NAT_GROUP_PUNCHTHROUGH_REQUEST,
	/// NATPunchthrough plugin: internal
	//ID_NAT_GROUP_PUNCHTHROUGH_REPLY,
	/// NATPunchthrough plugin: internal
	public const int ID_NAT_CONNECT_AT_TIME = 0x3b;
	/// NATPunchthrough plugin: internal
	public const int ID_NAT_GET_MOST_RECENT_PORT = 0x3c;
	/// NATPunchthrough plugin: internal
	public const int ID_NAT_CLIENT_READY = 0x3d;
	/// NATPunchthrough plugin: internal
	...
	public const int ID_NAT_TARGET_NOT_CONNECTED = 0x3e;
	...
	public const int ID_NAT_TARGET_UNRESPONSIVE = 0x3f;
	...
	public const int ID_NAT_CONNECTION_TO_TARGET_LOST = 0x40;
	/// NATPunchthrough plugin: This punchthrough is already in progress. Possibly the plugin is not installed.
	///  Bytes starting at offset 1 contains the RakNetGUID destination field of NatPunchthroughClient::OpenNAT().
	public const int ID_NAT_ALREADY_IN_PROGRESS = 0x41;
	...
	public const int ID_NAT_PUNCHTHROUGH_FAILED = 0x42;
	...
	public const int ID_READY_EVENT_QUERY = 0x47;

	/// Lobby packets. Second byte indicates type.
	public const int ID_LOBBY_GENERAL = 0x48;

	// RPC3, RPC4 error
	public const int ID_RPC_REMOTE_ERROR = 0x49;
	/// Plugin based replacement for RPC system
	public const int ID_RPC_PLUGIN = 0x4a;

	/// FileListTransfer transferring large files in chunks that are read only when needed, to save memory
	public const int ID_FILE_LIST_REFERENCE_PUSH = 0x4b;
	/// Force the ready event to all set
	public const int ID_READY_EVENT_FORCE_ALL_SET = 0x4c;

	/// Rooms function
	public const int ID_ROOMS_EXECUTE_FUNC = 0x4d;
	public const int ID_ROOMS_LOGON_STATUS = 0x4e;
	public const int ID_ROOMS_HANDLE_CHANGE = 0x4f;

	/// Lobby2 message
	public const int ID_LOBBY2_SEND_MESSAGE = 0x50;
	public const int ID_LOBBY2_SERVER_ERROR = 0x51;
	...
	public const int ID_FCM2_NEW_HOST = 0x52;
	/// \internal For FullyConnectedMesh2 plugin
	public const int ID_FCM2_REQUEST_FCMGUID = 0x53;
	/// \internal For FullyConnectedMesh2 plugin
	public const int ID_FCM2_RESPOND_CONNECTION_COUNT = 0x54;
	/// \internal For FullyConnectedMesh2 plugin
	public const int ID_FCM2_INFORM_FCMGUID = 0x55;
	/// \internal For FullyConnectedMesh2 plugin
	public const int ID_FCM2_UPDATE_MIN_TOTAL_CONNECTION_COUNT = 0x56;
	...
	/// \note Only the designated client gets this message
	public const int ID_FCM2_VERIFIED_JOIN_FAILED = 0x59;
	...
	public const int ID_FCM2_VERIFIED_JOIN_ACCEPTED = 0x5a;
	...
	/// \note Only the designated client gets this message
	/// \sa RespondOnVerifiedJoinCapable()
	public const int ID_FCM2_VERIFIED_JOIN_REJECTED = 0x5b;

	/// UDP proxy messages. Second byte indicates type.
	public const int ID_UDP_PROXY_GENERAL = 0x5c;

	/// SQLite3Plugin - execute
	public const int ID_SQLite3_EXEC = 0x5d;
	/// SQLite3Plugin - Remote database is unknown
	public const int ID_SQLite3_UNKNOWN_DB = 0x5e;
	/// Events happening with SQLiteClientLoggerPlugin
	public const int ID_SQLLITE_LOGGER = 0x5f;

	/// Sent to NatTypeDetectionServer
	public const int ID_NAT_TYPE_DETECTION_REQUEST = 0x60;
	/// Sent to NatTypeDetectionClient. Byte 1 contains the type of NAT detected.
	public const int ID_NAT_TYPE_DETECTION_RESULT = 0x61;

	/// Used by the router2 plugin
	public const int ID_ROUTER_2_INTERNAL = 0x62;
	...
	public const int ID_ROUTER_2_FORWARDING_NO_PATH = 0x63;
	...
	public const int ID_ROUTER_2_FORWARDING_ESTABLISHED = 0x64;
	...
	public const int ID_ROUTER_2_REROUTED = 0x65;

	/// \internal Used by the team balancer plugin
	public const int ID_TEAM_BALANCER_INTERNAL = 0x66;
	...
	public const int ID_TEAM_BALANCER_REQUESTED_TEAM_FULL = 0x67;
	...
	public const int ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED = 0x68;
	public const int ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED = 0x69;
	...
	public const int ID_TEAM_BALANCER_TEAM_ASSIGNED = 0x6a;

	/// Gamebryo Lightspeed integration
	public const int ID_LIGHTSPEED_INTEGRATION = 0x6b;

	/// XBOX integration
	public const int ID_XBOX_LOBBY = 0x6c;
	...
	public const int ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_SUCCESS = 0x6d;
	public const int ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_SUCCESS = 0x6e;
	...
	public const int ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_FAILURE = 0x6f;
	...
	public const int ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILURE = 0x70;
	...
	/// You can read the identifier used to challenge as follows:
	/// RakNet::BitStream bs(packet->data, packet->length, false); bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString password; bs.Read(password);
	public const int ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT = 0x71;
	/// \internal
	public const int ID_TWO_WAY_AUTHENTICATION_NEGOTIATION = 0x72;

	/// CloudClient / CloudServer
	public const int ID_CLOUD_POST_REQUEST = 0x73;
	public const int ID_CLOUD_RELEASE_REQUEST = 0x74;
	public const int ID_CLOUD_GET_REQUEST = 0x75;
	public const int ID_CLOUD_GET_RESPONSE = 0x76;
	public const int ID_CLOUD_UNSUBSCRIBE_REQUEST = 0x77;
	public const int ID_CLOUD_SERVER_TO_SERVER_COMMAND = 0x78;
	public const int ID_CLOUD_SUBSCRIPTION_NOTIFICATION = 0x79;

	// LibVoice
	public const int ID_LIB_VOICE = 0x7a;

	public const int ID_RELAY_PLUGIN = 0x7b;
	public const int ID_NAT_REQUEST_BOUND_ADDRESSES = 0x7c;
	public const int ID_NAT_RESPOND_BOUND_ADDRESSES = 0x7d;
	public const int ID_FCM2_UPDATE_USER_CONTEXT = 0x7e;
	public const int ID_RESERVED_3 = 0x7f;
	public const int ID_RESERVED_4 = 0x80;
	public const int ID_RESERVED_5 = 0x81;
	public const int ID_RESERVED_6 = 0x82;
	public const int ID_RESERVED_7 = 0x83;
	public const int ID_RESERVED_8 = 0x84;
	public const int ID_RESERVED_9 = 0x85;
